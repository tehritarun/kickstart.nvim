...
vim.keymap.set('n', '<F6>', function() require('dapui').toggle() end, { desc = 'Debug: See last session result.' })
...
local function debugpy_adapter_python()
  ...
  if vim.uv or vim.loop then return mason_python end
  return vim.fn.executable 'uv' == 1 and 'uv' or 'python3'
end

require('dap-python').setup(debugpy_adapter_python())
